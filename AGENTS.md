# Beeswax Theme - Agent Rules

This document contains coding rules and conventions for the Beeswax theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in Beeswax theme components.
...
{{
  include(
    '@beeswax/components/icon/icon.twig',
    {
      weight: 'bold',
      icon: icon
    }
...
```twig
{% include '@beeswax/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the Beeswax theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the Beeswax theme directory (`web/themes/custom/beeswax/`). Running these commands ensures that:
